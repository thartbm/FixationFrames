...
def doTrial(cfg):
    ...
    while waiting_for_response:
        ...
        #     if showFixation:
        #         if cfg['hw']['tracker'].gazeInFixationWindow(fixloc=[frameoffset,-2]):
        #             showStimulus = True
        #         else:
        #             showStimulus = False
        #     else:
        ...
        if one_on and one_prepped and not(one_active):
            # switch motor 1 on
            one_active = True
            print('flip on motor 1')
            # cfg['hw']['tactor'].go(motor=0)
        
        if one_active and not(one_on):
            # pause over, prep motor 2:
            one_active = False
            # cfg['hw']['tactor'].prep(motor=1, duration=133, strength=127)
            two_prepped = True
            one_prepped = False

        if two_on and two_prepped and not(two_active):
            # switch motor 2 on
            two_active = True
            print('flip on motor 2')
            
            # cfg['hw']['tactor'].go(motor=1)

        if two_active and not(two_on):
            # pause over, prep motor 2:
            two_active = False
            # cfg['hw']['tactor'].prep(motor=0, duration=133, strength=127)
            one_prepped = True
            two_prepped = False
        ...
        # key responses:
        keys = event.getKeys(keyList=['space','escape','0', 'left', 'right', 'num_left', 'num_right'])
        if len(keys):
            if 'space' in keys:
                waiting_for_response = False
            ...
            if 'escape' in keys:
                cleanExit(cfg)
            if 'left' in keys or 'num_left' in keys:
                answer = 0
            if 'right' in keys or 'num_right' in keys:
                answer = 1
    ...
    response['answer']      = answer # 0 = left; 1 = right?
...
def getStimuli(cfg, setup='tablet'):
    ...
    cfg['hw']['tactor'] = tactor(port = '/dev/ttyACM0', baudrate=115200)
...
class tactor:
    ...
    def __init__(self, port, baudrate):
         """
         port: where to connect, e.g.: '/dev/ttyACM0'
         baudrate: connection speed, e.g.: 115200
         """

        self.connection = Serial( port     = port,
                                  baudrate = baudrate          )
    ...
    def prep(self, motor=2, duration = 133, strength=127):

        """
        motor: which stimulator to use? (0=first, 1=second, 2=both)
        duration: how long should the motor(s) be active (in miliseconds)
        strength: strength level between 0 and 127 (0 = none, 127 = max, 128 = 126, 129 = 125, etc)
        """
        ...
        self.connection.write(bytes(prepcommand, 'utf-8'))


    def go(self, motor=None):

        """
        motor: which stimulator to use? (0=first, 1=second, 2=both)
        not sure if this can still be set or not
        """
        ...
        self.connection.write(bytes(gocommand, 'utf-8'))
