...
def doDotTrial(cfg):
    ...
    if 'dotoffset' in trialdict.keys():
        dotoffset = trialdict['dotoffset']
    else:
        dotoffset = 1


    if 'probetype' in trialdict.keys():
        probetype = trialdict['probetype']
    else:
        probetype = 'flashed'
    ...
    frameoffset = [-8,-8]
    cfg['hw']['bluedot'].pos = [frameoffset[0],frameoffset[1]+dotoffset]
    cfg['hw']['reddot'].pos = [frameoffset[0],frameoffset[1]-dotoffset]
    ...
    while waiting_for_response:
        ...
        # use for dark-dot offset as well
        darkdotOffset = (offsetX/distance) * dotoffset * -2
        ...
        if showdark:
            cfg['hw']['darkdot'].pos = [frameoffset[0],frameoffset[1]+darkdotOffset]
            cfg['hw']['darkdot'].draw()
        ...
        cfg['hw']['bluedot_ref'].pos = [ (-1*frameoffset[0])+percept, (-1*frameoffset[1])+dotoffset ]
        cfg['hw']['reddot_ref'].pos = [  (-1*frameoffset[0])-percept, (-1*frameoffset[1])-dotoffset ]
        if cfg['expno'] in [2,3]:
            pass
        else:
...
def getStimuli(cfg, setup='tablet'):
    ...
    cfg['hw']['reddot'] = visual.Circle(win=cfg['hw']['win'],
                                         ...
                                         fillColor=[1,-1,-1],
                                         pos=[0-cfg['stim_offsets'][0],-dot_offset-cfg['stim_offsets'][1]])
    cfg['hw']['darkdot'] = visual.Circle(win=cfg['hw']['win'],
                                         units='deg',
                                         size=[1,1],
                                         edges=180,
                                         lineWidth=0,
    ...
    cfg['hw']['frame'] = visual.ShapeStim(  win=cfg['hw']['win'],
                                            ...
                                            lineColor=None,
                                            fillColor=[1,1,1],
                                            vertices=[[[-3.5,-3.5],[-3.5,3.5],[-3,3.5],[-3,-3.5],[-3.5,-3.5]],
                                                      [[ 3.5,-3.5],[ 3.5,3.5],[ 3,3.5],[ 3,-3.5],[ 3.5,-3.5]]]
                                            )  
...
def getTasks(cfg):
    ...
    if cfg['expno']==1:
        ...
        condictionary = [
                 ...
                 {'period':1/3, 'amplitude':4, 'stimtype':'classicframe', 'dotoffset':2, 'probetype':'flashed'},
                 {'period':1/3, 'amplitude':4, 'stimtype':'classicframe', 'dotoffset':2, 'probetype':'continuous'},

                 {'period':1/3, 'amplitude':4, 'stimtype':'classicframe', 'dotoffset':4, 'probetype':'flashed'},
                 {'period':1/3, 'amplitude':4, 'stimtype':'classicframe', 'dotoffset':4, 'probetype':'continuous'},
