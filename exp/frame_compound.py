...
def doDotTrial(cfg):
    ...
    if 'maxdotlife' in trialdict.keys():
        # either from trialdict:
        maxdotlife = trialdict['maxdotlife']
    else:
        # or the default value from the stimulus definition:
    ...
    # like dotsize?
    dotsize = cfg['hw']['dotfield']['dotsize']

    # TIMESPREAD
    if 'timespread' in trialdict.keys():
        timespread = trialdict['timespread']
    else:
    ...
    extra_frames = 12
    # a 200 ms pause?
    # yes: even with a 200 ms time-spread
    # the probes will still be flashed with all dots stationary!
    ...
    xpos = np.linspace(-0.5,0.5,ndots) * (framesize[0] - dotsize)
    random.shuffle(xpos)
    ...
    while waiting_for_response:
        ...
        # flash the dots, if/when necessary:
        if flash_red:
            cfg['hw']['reddot'].draw()
        if flash_blue:
            cfg['hw']['bluedot'].draw()
...
def runTasks(cfg):
    ...
    while cfg['currentblock'] < len(cfg['blocks']):
        ...
        while cfg['currenttrial'] < len(cfg['blocks'][cfg['currentblock']]['trialtypes']):
            ...
            if trialdict['stimtype'] in ['compoundframe']:
    ...
    if cfg['expno']==5:
        ...
        condictionary = [
                ...
                #  {'test':'anglespread', 'period':1/4, 'amplitude':4, 'stimtype':'compoundframe', 'timespread':0, 'anglespread':40, 'amplitudespread':0, 'framesize':[7,7], 'maxdotlife':1/2},
                 ...
                 {'test':'combined', 'period':1/4, 'amplitude':4, 'stimtype':'compoundframe', 'timespread':0.200, 'anglespread':90, 'amplitudespread':16.0, 'framesize':[7,7], 'maxdotlife':1/8},
                 {'test':'combined', 'period':1/4, 'amplitude':4, 'stimtype':'compoundframe', 'timespread':0.200, 'anglespread':90, 'amplitudespread':20.0, 'framesize':[7,7], 'maxdotlife':1/8},
