...
df3$percept <- df3$percept_abs * 2 * df3$xfactor
...
probeLagLookUp <- function(framelags, FMD, FMA, pause_duration=NULL, probe_duration=NULL) {
  ...
  for (lagdur in lagdurations) {
    ...
    if (probe_onset < HPD) {
      ...
      if (probe_offset <= HPD) {
        ...
        static_part <- (HPD - probe_onset) / (2*probe_duration)
        
        dynamic_duration <- probe_offset - HPD
...
first fix the lead/lag test tags

```{r}
cfg <- RJSONIO::fromJSON("data/apparent/exp_5/mariusB/cfg.json")
...
for (blockno in c(1:length(blocks))) {
  ...
  trialtypes <- blocks[[blockno]]$trialtypes
  
  for (trialno in c(1:length(trialtypes))) {
    trialtype <- trialtypes[[trialno]]
    condition <- conditions[[trialtype+1]]
    
    test_column <- c(test_column, condition$test)
  }
  
  
}
write.csv(data.frame(test=test_column),
          file="data/apparent/exp_5/mariusB_org/test_column.csv",
          row.names=FALSE)
...
```{r probe-lead-lag, fig.width=8, fig.height=8}
dfB <- read.csv("data/apparent/exp_5/mariusB_org/responses.csv", stringsAsFactors = FALSE)
dfB$percept <- dfB$percept_abs * 2 * dfB$xfactor
layout(mat=matrix(c(1,2), nrow=2, ncol=1), heights=c(1,1))

for (movdur in c(0.2, 0.3)) {
...
```{r probe-lead-lag, fig.width=5, fig.height=10}
dfB <- read.csv("data/apparent/exp_6/mariusB/responses.csv", stringsAsFactors = FALSE)
# dfB$percept <- dfB$percept_abs * 2 * dfB$xfactor
layout(mat=matrix(c(1,2,3,4), nrow=4, ncol=1), heights=c(1,1))

for (movdur in c(0.1, 0.2, 0.4, 0.6)) {
...
```{r probe-duration, fig.width=5, fig.height=7.5}
dfB <- read.csv("data/apparent/exp_7/probeDurs/responses.csv", stringsAsFactors = FALSE)

dfB$probe_frames <- dfB$probeduration / (1/30)


layout(mat=matrix(c(1,2,3), nrow=3, ncol=1), heights=c(1,1))

for (probdur in c(sort(unique(dfB$probe_frames)))) {
  ...
  for (rel in c('lead','lag')) {
    idx <- which(dfB$probe_frames == probdur & dfB$test == rel)
    ...
    for (stimtype in c('classicframe')) {
    ...
    text(x=x_offset, y=5, labels=sprintf('%ss - %d ms probes', rel, round((probdur/30)*1000)), xpd=TRUE)
